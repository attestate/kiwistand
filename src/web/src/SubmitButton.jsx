...
const SubmitButton = () => {
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (titleInput) {
        titleInput.removeEventListener('input', () => setTitle(titleInput.value));
      }
    }
  }, []);
  ...
  const handleClick = async (e) => {
    ...
    if (title.length > 80 || url.length > 2048) {
      showMessage("The title should be no more than 80 characters, and the URL should be no more than 2048 characters.");
      setIsLoading(false);
      return;
    }
    if (title.length === 0) {
    ...
    let signature;
    try {
    signature = await signer._signTypedData(API.EIP712_DOMAIN, API.EIP712_TYPES, value);
    } catch(err) {
      console.log(err);
  ...
  return (
    <button style={buttonStyles} onClick={handleClick} disabled={isLoading && !isError}>
